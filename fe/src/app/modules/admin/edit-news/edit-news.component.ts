...
export class EditNewsComponent implements OnInit {
    ...
    loadNewsDetail(slug: string) {
        this._newsService.getNewsBySlug(slug).subscribe(
            (news) => {
                this.news = news;
                this.selectedCategoryIds = this.news.categories?.map((c: any) => c.id) || [];
            },
            (error) => {
    ...
    removeCategory(catId: number): void {
        ...
        this.news.categories = this.selectedCategoryIds;
    }
    ...
    submitUpdateNews() {
        this.errorFields = {};
        if (this.news.title && this.news.title.length > 100) {
        ...
        if (this.news.thumbnail) {
            // Kiểm tra đuôi file ảnh nếu là nhập url
            const allowedExt = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
            const urlExt = this.news.thumbnail
                .split('.')
                .pop()
                ?.toLowerCase()
                .split('?')[0];
        ...
        this._newsService.updateNews(this.news?.id, this.news).subscribe(
            (news) => {
                ...
                this._router.navigate(['/admin/news', `${newSlug}.html`]);
