...
export class FacilitiesComponent implements OnInit {
  fields: FieldConfig[] =[
    ...
    {
        name: 'name',
        labelKey: 'facilities.name',
        type: 'text',
        placeholderKey: 'facilities.enterFullName',
        required: true,
        
    },
    {
        name: 'description',
        labelKey: 'facilities.description',
        type: 'text',
        placeholderKey: 'facilities.enterdDescription',
        required: true,
  ...
  user:User
  selectedDes:Facilities = null;
  selectedIds: string[] = [];
  amenities: Facilities[] = [];
  hasSelectedFacilities:boolean= false;
  ...
  loadFacilitiess(): void {
    this.loading = true;
    const payload = this.getPayload();

    this.facilitiesService.getFacilities(payload).subscribe({
      next: (response) => {
        console.log("facilities res:",response)
      ...
      error: (error) => {
        ...
        this._alertService.showAlert({
          title: 'Error',
          message: 'Failed to load facilitiess',
          type: 'error'
        });
        console.error('Error loading facilitiess:', error);
      }
    });
  }
  debounceSearchFunc(){
  ...
  loadAutocompleteOptions(): void {
      ...
      this.facilitiesService.getFacilities({ page_index: 1, page_size: 1000 }).subscribe({
          next: (response) => {
              const nameField = this.filterFields.find(f => f.name === 'name');
              if (nameField) {
                  nameField.autocompleteOptions = response.data.map((dest: Facilities) => dest.name);
              }
          },
          error: (error) => {
  ...
  async toggleEditUserDrawer(facilities?: Facilities) {
      if (facilities) {
        this.selectedDes=facilities;
      }
      ...
      if (this.showEditUser) {
        ...
        componentRef.instance.titleKey = 'facilities.detail';
        ...
        componentRef.instance.loadData = this.loadFacilitiess.bind(this);
  ...
  saveHandler(payload: any): Observable<any> {
    return this.facilitiesService.updateFacilities(payload.get('uuid'), payload);
  }

  addSaveHandler(payload: any): Observable<any> {
    return this.facilitiesService.createFacilities(payload);
  }
  deleteHandler(id: string): Observable<any> {
    this.selectedIds=[]
    this.hasSelectedFacilities = false;
    return this.facilitiesService.deleteFacilities(id);
  }
  ...
  toggleAllRows(event: Event): void {
      const checked = (event.target as HTMLInputElement).checked;
        this.amenities.forEach((facilities) => (facilities.selected = checked));
        this.toggleRow();
  }
  ...
  toggleRow(): void {
    this.selectedIds = this.amenities
            ...
            .map((user) => user.uuid);
    this.hasSelectedFacilities = this.selectedIds.length >= 1;
  }
}