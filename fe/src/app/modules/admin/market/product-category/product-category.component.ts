...
export class ProductCategoryComponent implements OnInit {
    ...
    // status
    statusOptions = this._productCategoryService.getStatus();
    ...
    loadProductCategories() {
        ...
        this._productCategoryService.getProductCategories(payload).subscribe(([categories_parent, totalProductCategory]) => {
            ...
            this.totalPages = Math.ceil(this.totalProductCategory / this.pageSize);
        });
        this.checkboxSelected = [];
    }
    ...
    createCategory(): void {
        ...
        this._productCategoryService.createProductCategory(this.newCategory).subscribe({
            next: () => {
                this.closeAddCategoryPopup();
                this._productCategoryService.getProductCategories(this.getPayload()).subscribe(([categories_parent, totalProductCategory]) => {
                ...
                this._alertService.showAlert({
                    title: this.translocoService.translate('product_category.success_title'),
                    message: this.translocoService.translate('product_category.success_create_message'),
                    type: 'success'
                });
            },
            error: (err) => {
    ...
    openUpdatePopup(category: any) {
        if (!category) {
            this._alertService.showAlert({
                title: this.translocoService.translate('product_category.error_title'),
                message: this.translocoService.translate('product_category.error_select_category'),
                type: 'error'
            });
            return;
        }
    ...
    submitUpdateCategory(): void {
        ...
        this._productCategoryService.updateProductCategory(this.selectedCategoryId, this.newCategory).subscribe({
            next: () => {
                ...
                this._alertService.showAlert({
                    title: this.translocoService.translate('product_category.success_title'),
                    message: this.translocoService.translate('product_category.success_update_message'),
                    type: 'success'
                });
            },
            error: (err) => {
    ...
    createChildCategory(): void {
        ...
        this._productCategoryService.createProductCategory(this.newCategory).subscribe({
            next: () => {
                ...
                this._productCategoryService.getProductCategories(this.getPayload()).subscribe(([categories_parent, totalNewsCategory]) => {
                    this.categories_parent = categories_parent;
                    for (const cat of this.categories_parent) {
                        this.categories_child = this.categories_child.concat(cat.children);
                    }
    ...
    deleteCategory() {
        if (!this.selectedCategoryId) return;
        this._productCategoryService.deleteProductCategory(this.selectedCategoryId).subscribe({
            next: () => {
                ...
                this._alertService.showAlert({
                    title: this.translocoService.translate('product_category.success_title'),
                    message: this.translocoService.translate('product_category.success_delete_message'),
                    type: 'success'
                });
            },
            error: (err) => {
    ...
    deleteAllCategories() {
        this.checkboxSelected.forEach(category => {
            this._productCategoryService.deleteProductCategory(category.id).subscribe({
                next: () => {
    ...
    toggleDropdown(catId: string): void {
        if (this.openDropdowns.has(catId)) {
            this.openDropdowns.delete(catId);
        } else {
            this.openDropdowns.clear();
            this.openDropdowns.add(catId);
        }
    }

    isDropdownOpen(catId: string): boolean {
        return this.openDropdowns.has(catId);
    }

    changeStatus(cat: any, catStatusId: number): void {
        if (cat.status !== catStatusId) {
            this._productCategoryService.updateProductCategory(cat.id, { status: catStatusId }).subscribe({
                next: (res) => {
                    this.loadProductCategories();
                    this._alertService.showAlert({
                        title: 'Thành công',
            ...
            cat.status = catStatusId; // Cập nhật local luôn nếu muốn
        }
        this.checkboxSelected = [];
        this.closeDropdown(cat.id);
    }

    closeDropdown(catId: string): void {
        this.openDropdowns.delete(catId);
    }

    changeHighLight(cat: any, highlight: boolean) {
        console.log('highlight', highlight)
        console.log('cat', cat)
        this._productCategoryService.updateProductCategory(cat.id, { highlight }).subscribe({
            next: () => {
                this.loadProductCategories();
            },
        });    
