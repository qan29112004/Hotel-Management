...
@Component({
  ...
  templateUrl: './feedback.component.html',
      styleUrls: ['./add-feedback/add-feedback.component.scss'] 
...
loadFeedback(): void {
  ...
  if (this.role.toLowerCase() === 'user') {
    payload.filterRules = payload.filterRules || [];
...
onClickOutside(event: MouseEvent): void {
  ...
  if (this.selectedIdsToDelete.length > 0) {
    this.showDeleteForm = true; // chỉ mở popup, chưa reset gì
  }
}
