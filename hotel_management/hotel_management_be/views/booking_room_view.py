...
@auto_schema_post(BookingRoomSerializer)
@permission_classes([IsAdminUser])
@api_view(['POST'])
def add_BookingRoom(request):
    try:
        
        serializers = BookingRoomSerializer(data=request.data, context={'request':request})
        if serializers.is_valid():
            new_BookingRoom = serializers.save(created_by = request.user)
            return AppResponse.success(SuccessCodes.CREATE_AMENITY, data={"data":BookingRoomSerializer(new_BookingRoom).data})
...
@auto_schema_patch(BookingRoomSerializer)
@permission_classes([IsAdminUser])
@auto_schema_delete(BookingRoomSerializer)
@permission_classes([IsAdminUser])
@api_view(['PATCH', 'DELETE'])
def BookingRoom_detail(request, uuid):
    try:
        BookingRoom = BookingRoom.objects.get(uuid__icontains=uuid)

        if request.method == 'PATCH':
            
            serializer = BookingRoomSerializer(BookingRoom, data=request.data, partial=True)
            if serializer.is_valid():
                with transaction.atomic():
                    updated = serializer.save(updated_by=request.user)
                return AppResponse.success(SuccessCodes.UPDATE_AMENITY, data={"data": BookingRoomSerializer(updated).data})
        ...
        return AppResponse.error(ErrorCodes.NOT_FOUND, "BookingRoom not found")
    except Exception as e:
...
@permission_classes([IsAdminUser])
@api_view(['POST'])
def list_BookingRoom(request):
    try:
        list_BookingRoom = BookingRoom.objects.all()
        paginated_BookingRoom, total = Querykit.apply_filter_paginate_search_sort(request=request, queryset=list_BookingRoom).values()
        serializers = BookingRoomSerializer(paginated_BookingRoom, many=True)
